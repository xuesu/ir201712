...
class BaseSpider(abc.ABC):
    ...
    def get_response(self, referer, url, encoding='utf-8', proxy=False, retry_time=3, delay_time=1):
        '''
            通过给定的参数设置headers,获取url的返回内容
        parameters:
            referer:设置headers.Referer的内容
            url:要爬取的url
            format1,format2:url中替换的内容（如果要替换的话）
        return:
            response(网页返回的)
        '''
        self.headers['User-Agent'] = self.user_agents[random.randint(0, len(self.user_agents) - 1)]
        self.headers['Referer'] = referer
        for rt in range(retry_time):
            try:
                # if not proxy:
                ...
                with requests.get(url, headers=self.headers, timeout=30) as resp:
                    ...
                    # .Session().
                    # else:
                    # response = requests.get(url,proxies = Util.Get(),headers=self.header_dict[headers], timeout=20)
